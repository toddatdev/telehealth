@if (!empty($specialities))
    <div class="dc-services-holder dc-aboutinfo">
        <div class="dc-infotitle">
            <h3 class="profilerightsidetitle">{{ trans('lang.offered_services') }}</h3>
        </div>
		...
		<div id="dc-accordion" class="dc-accordion" role="tablist" aria-multiselectable="true" style="padding: 15px 10px; border-bottom: 1px dashed #ddd;">
			<figure class="dc-titleicon" style="margin-right: 0px;"><img src="{{ asset(Helper::getImage('uploads/specialities', $speciality->image, '', 'speciality-default.png')) }}" alt="{{ trans('lang.img_desc') }}"></figure>
			<span class="servicetitle" style="vertical-align: text-top;">{{ html_entity_decode(clean($speciality->title)) }}</span>
			<span class="priceimtes" style="margin-left: 20px; vertical-align: text-top;">{{ !empty($symbol['symbol']) ? $symbol['symbol'] : '$' }}{{ clean($service_data['price']) }}</span>
		</div>
            @endif
        @endforeach
    </div>
